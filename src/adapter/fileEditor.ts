/**
 * File Editor for MCP Adapter
 * Uses AST manipulation to inject code into test files
 * Copied from server/fileEditor.ts for adapter architecture
 */
...
export class FileEditor {
  ...
  async removeConnect(testFile: string): Promise<void> {
    return this.withFileLock(testFile, async () => {
      ...
      try {
        ...
        if (!connectCall) {
          console.error(`[testing-mcp:adapter] No connect() call found in ${testFile}`);
          return;
        }
        ...
        console.error(`[testing-mcp:adapter] Removed connect() from ${testFile}`);
      } finally {
  ...
  async removeMarkers(testFile: string): Promise<void> {
    return this.withFileLock(testFile, async () => {
      ...
      console.error(`[testing-mcp:adapter] Removed markers from ${testFile}`);
    });
  }
  ...
  async restoreFromBackup(testFile: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[testing-mcp:adapter] Restored ${testFile} from backup`);
    } catch (error) {
      console.error(`[testing-mcp:adapter] Failed to restore from backup:`, error);
    }
  }
}
