/**
 * MCP Tools for Testing-MCP Adapter
 * Uses BridgeClient to communicate with daemon
 */
...
/**
 * MCP Tools Implementation using BridgeClient
 */
export class MCPTools {
   ...
   */
  async getCurrentTestState(
    params: z.infer<typeof GetCurrentStateSchema>
  ): Promise<MCPToolResponse> {
    try {
      const state = await this.bridgeClient.getCurrentState({
      ...
      if (!state) {
        return {
          success: false,
          error:
            "No active test connection found. Please ensure a test is running with await connect().",
        };
      }
   ...
   */
  async finalizeTest(
    params: z.infer<typeof FinalizeTestSchema>
  ): Promise<MCPToolResponse> {
    try {
      const { testFile, removeMarkers } = params;

      // Get the connection info before removing
      const { connections } = await this.bridgeClient.listConnections();
      ...
      if (connection) {
        await this.bridgeClient.sendClose({
          testFile: connection.testFile,
  ...
  async executeTestStep(
    params: z.infer<typeof ExecuteTestStepSchema>
  ): Promise<MCPToolResponse> {
    try {
      // Get current state to find which test to execute in
      const currentState = await this.bridgeClient.getCurrentState({
        testFile: params.testFile,
        testName: params.testName,
      });
      ...
      if (!currentState) {
        return {
          success: false,
          error:
            "No active test connection. Ensure test is running with await connect().",
        };
      }
      ...
      console.error(
        `[testing-mcp:adapter] Executing code in test: ${testFile} - ${testName}`
      );
      console.error(`[testing-mcp:adapter] Code to execute:\n${params.code}`);

      // Send execute command to daemon and wait for response
      const newState = await this.bridgeClient.sendExecute({
        testFile,
        testName,
        code: params.code,
        timeout: 30000,
